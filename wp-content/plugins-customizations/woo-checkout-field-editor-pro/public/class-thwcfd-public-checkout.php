...
class THWCFD_Public_Checkout {
	...
	public function define_public_hooks(){
		...
		// Перехватываем сообщения об ошибках WooCommerce и убираем префикс "Выставление счета"
		add_filter('woocommerce_checkout_field_error_message', array($this, 'remove_billing_prefix_from_error'), 999, 2);
		add_action('woocommerce_after_checkout_validation', array($this, 'clean_billing_errors'), 999, 2);
	...
	*************************************/
	...
	public function add_validation_error($msg, $errors=false){
		// Убираем префикс "Выставление счета" или "Выставление счёта" из сообщений об ошибках
		$msg = preg_replace('/^Выставление\s+счета\s+/iu', '', $msg);
		$msg = preg_replace('/^Выставление\s+счёта\s+/iu', '', $msg);
	...
	*****************************************/
	...
	/**
	 * Убираем префикс "Выставление счета" из сообщений об ошибках
	 */
	public function remove_billing_prefix_from_error($message, $field_key) {
		// Поля billing, для которых нужно убрать префикс
		...
		if (in_array($field_key, $billing_fields)) {
			...
			$message = preg_replace('/^Выставление\s+счета\s+/iu', '', $message);
			$message = preg_replace('/^Выставление\s+счёта\s+/iu', '', $message);
		}
	...
	/**
	 * Очищаем все ошибки billing от префикса "Выставление счета"
	 */
	public function clean_billing_errors($posted, $errors) {
		...
		foreach ($error_codes as $code) {
			if (in_array($code, $billing_fields)) {
