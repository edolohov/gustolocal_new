			...
			foreach ( $fieldset as $key => $field ) {
				...
				switch ( $fieldset_key ) {
					case 'shipping':
						...
						break;
					case 'billing':
						// Не добавляем префикс "Billing" для определенных полей
						$fields_without_billing_prefix = array( 'billing_first_name', 'billing_last_name', 'billing_phone' );
						if ( ! in_array( $key, $fields_without_billing_prefix, true ) ) {
							/* translators: %s: field name */
