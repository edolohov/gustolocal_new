...
function wmb_default_settings(){
  ...
  return [
    ...
    'delivery' => [
      'tuesday' => [
        'enabled'  => true,
        'deadline' => ['dow' => 0, 'time' => '14:00'], // Вс 14:00
      ],
      'friday' => [
        'enabled'  => true,
        'deadline' => ['dow' => 3, 'time' => '14:00'], // Ср 14:00
      ],
      'timezone' => $tz,
      'blackout' => [],
...
function wmb_page_items(){
  ...
  if (!empty($_POST['wmb_items_nonce']) && wp_verify_nonce($_POST['wmb_items_nonce'],'wmb_items_save')){
    if (!empty($_POST['bulk_action']) && !empty($_POST['ids']) && is_array($_POST['ids'])){
      ...
      elseif ($_POST['bulk_action']==='delete'){ foreach($ids as $id) wp_delete_post($id, true); } // жесткое удаление
  ...
  /* --- компактная таблица со складным блоком --- */
  echo '<style>
  ...
  // JS: раскрытие + авто-высота текстовых областей
  echo '<script>
    (function(){
...
/* ---------- Import CSV (Название,Цена,Единица,Категория,Теги,Состав,Аллергены,Активно) ---------- */
...
/* ---------- Settings page (описание + доставка) ---------- */
...
/* ---------- REST: /wmb/v1/menu (ingredients + allergens) ---------- */
...
usort($out_sections, function($a, $b) use ($index) {
  ...
  if ($ai === $bi) return strcmp($a['title'], $b['title']); // одинаковый приоритет — по алфавиту
  return $ai - $bi;
});
...
function wmb_ajax_add_to_cart(){
  ...
  foreach($items as $id=>$qty){
    ...
    if ($post_id>0){ $name=get_the_title($post_id); $price=floatval(get_post_meta($post_id,'wmb_price',true)); $kcal=0; }
    else { $name=$id; $price=0; $kcal=0; }
    $total_price += $price*$qty;
    $items_list[] = ['id'=>$id,'name'=>$name,'kcal'=>$kcal,'price'=>$price,'qty'=>$qty];
  }
